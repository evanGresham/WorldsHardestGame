...
void setEdges() {
  for (int i = 1; i< tiles.length-1; i++) {
    for (int j = 1; j< tiles[0].length-1; j++) {
      if (tiles[i][j].wall) {
        if (!tiles[i+1][j].wall) {
          tiles[i][j].wallSides.add(1);
        }
        if (!tiles[i][j+1].wall) {
          tiles[i][j].wallSides.add(2);
        }
        if (!tiles[i-1][j].wall) {
          tiles[i][j].wallSides.add(3);
        }
        if (!tiles[i][j-1].wall) {
          tiles[i][j].wallSides.add(4);
        }
      }
    }
  }
...
void setDots() {
  dots.add(new Dot(tiles[6][3],tiles[15][3],1));
  dots.add(new Dot(tiles[6][5],tiles[15][5],1));
  dots.add(new Dot(tiles[15][4],tiles[6][4],-1));
  dots.add(new Dot(tiles[15][6],tiles[6][6],-1));
